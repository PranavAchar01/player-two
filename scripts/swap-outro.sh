#!/bin/zsh
# Drops a fresh recording of the five sponsor lines into the video and re-renders it (about 3 minutes).
# usage: scripts/swap-outro.sh ~/Downloads/<recording>.m4a
set -e
cd "${0:A:h}/.."
...
uv run --quiet --python 3.12 --with mlx-whisper python scripts/new-outro.py "$1"
