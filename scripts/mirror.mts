...
for (const id of process.argv.slice(2)) {
  ...
  const win = Math.min(n - 2, Math.round(SECONDS * track.fps));
  ...
  function fly(leadTicks: number) {
    ...
    let yaw = 0;
    const ctrl: number[][] = [], body: BodyCommand[] = [], state: number[][] = [], skeleton: number[][][] = [];
    ...
    for (let t = 0; t < ticks; t++) {
      ...
      const wantYaw = Math.max(-0.6, Math.min(0.6, Math.atan2(-hips.x, hips.y) * 0.7));
      ...
      for (const [side, ids] of [["left", ["24", "26", "28"]], ["right", ["23", "25", "27"]]] as [Side, string[]][]) {
        ...
        // Single-camera depth cannot tell which leg is in front when the dancer turns away, which would cross
        ...
        const thigh = keepApart(rotZ(toRobotFrame(sub(k, h)), -yaw)), shin = keepApart(rotZ(toRobotFrame(sub(a, k)), -yaw));
      ...
      skeleton.push(IDS.map((k) => [lerp(image[k][0], shown), lerp(image[k][1], shown)]));
      if (out.held) held++;
  ...
  await writeFile(`data/demos/mirror-${id}.json`, JSON.stringify({ source: `https://mixkit.co (clip ${id.slice(7)}, Mixkit free license)`, video: id, aspect, startSeconds, task: null, ctrl: final.ctrl, body: final.body, skeleton: final.skeleton, stats: { tracked: 100 - pct(final.held), limited: pct(final.limited), lagMsBefore: lag * 20, lagMsAfter: residual * 20 } }));
  console.log(id, `window ${startSeconds.toFixed(1)}s`, `| tracking lag ${lag * 20} ms -> ${residual * 20} ms with ${lag * 20} ms of preview`, `| speed cap active ${pct(final.limited)}% (was 6 rad/s, now ${MIRROR_SPEED_LIMIT})`, `| legs scale ${metresPerUnit.toFixed(2)} m/unit`);
}
